...
$(document).ready(function () {
    $('#supres-run').click(function () {
        ...
        var view_long_signal = document.getElementById('view_long_signal');
        var view_short_signal = document.getElementById('view_short_signal');
        // 設定form格式儲存參數
        ...
        if (view_long_signal.style.visibility === 'hidden') {
            view_long_signal.style.visibility = '';
        }
        if (view_short_signal.style.visibility === 'hidden') {
            view_short_signal.style.visibility = '';
        }
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                var longsignal_table = $('#LongSignalTable').DataTable();
                var shortsignal_table = $('#ShortSignalTable').DataTable();
                ...
                longsignal_table.clear().draw();
                shortsignal_table.clear().draw();

                // long_signal 的report格式
                if (res['long_signal'] != []) {
                    for (var i = 0; i < res['long_signal'].length; i++) {
                        var rowData = res['long_signal'][i];
                        var row = [
                            rowData[2],
                            rowData[0],
                            'Long',
                        ...
                        longsignal_table.row.add(row).draw();
                    }
                }
                // short_signal 的report格式
                if (res['short_signal'] != []) {
                    for (var i = 0; i < res['short_signal'].length; i++) {
                        var rowData = res['short_signal'][i];
                        var row = [
                            rowData[2],
                            rowData[0],
                            'Short',
                        ...
                        shortsignal_table.row.add(row).draw();
                    }
                }
                // gap 的report格式

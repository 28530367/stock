function make_track_list_Table(track_list_Table, res) {
    ...
    if (res != []) {
        for (var i = 0; i < res.length; i++) {
            ...
            var row = [
                ...
                rowData['symbol'],
                rowData['start_date'],
                rowData['number_of_signals'],
                rowData['up_gap_interval'],
                rowData['diff'],
                rowData['peak_left'],
                rowData['peak_right'],
                rowData['valley_left'],
                rowData['valley_right'],
                rowData['swap_times'],
                rowData['previous_day'],
                rowData['survival_time'],
                rowData['nk_valley_left'],
                rowData['nk_valley_right'],
                rowData['nk_peak_left'],
                rowData['nk_peak_right'],
                rowData['nk_startdate'],
                rowData['nk_enddate'],
                rowData['nk_interval'],
                rowData['nk_value'],
                '<td><button class="btn btn-danger btn-untrack">Untrack</button></td>',
                '<td><button class="btn btn-success btn-run_analysis">Run analysis</button></td>',
            ];
            track_list_Table.row.add(row).draw();

            $('.btn-untrack').on('click', function () {
                
                var rowData = track_list_Table.row($(this).closest('tr')).data();
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        track_list_Table.clear().draw();
                        make_track_list_Table(track_list_Table, res)
                    }
                });
            });

            $('.btn-run_analysis').on('click', function () {
                const loadingContainer = $('#loading-container');
                const loadingbackground = $('#loading-background')
                loadingContainer.show();
                loadingbackground.show()

                var static_Backdrop = document.getElementById('staticBackdrop');
                ...
                var symbol = rowData[1];
                var start_date = rowData[2];
                var signals_selected_values = rowData[3];
                var gap_interval = rowData[4];
                var closeness_threshold = rowData[5];
                var peak_left = rowData[6];
                var peak_right = rowData[7];
                var valley_left = rowData[8];
                var valley_right = rowData[9];
                var swap_times = rowData[10];
                var previous_day = rowData[11];
                var time_interval = rowData[12];
                var nk_valley_left = rowData[13];
                var nk_valley_right = rowData[14];
                var nk_peak_left = rowData[15];
                var nk_peak_right = rowData[16];
                var nk_startdate = rowData[17];
                var nk_enddate = rowData[18];
                var nk_interval = rowData[19];
                var nk_value = rowData[20];
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    static_Backdrop.style.visibility = '';
...
$(document).ready(function (){
    var track_list_Table = $('#TrackListTable').DataTable({
        "scrollX": true,  // 水平滾動
    });
    track_list_Table.clear().draw();
    ...
    $('.clossbutton').on('click', function () {
        static_Backdrop.style.visibility = 'hidden';
    });

...
class Mega(ExtractPdf):
    ...
    def __init__(self, file_path):
        ...
        self.possible_rating = ['逢低買進', '區間操作', '買進']
    ...
    def get_rating_report_version_1(self, page):
        clip_report_version_1= fitz.Rect(400, 0, page.rect.width, 200)
        text_report_version_1 = page.get_text(clip=clip_report_version_1, sort=True).strip()
        try:
    ...
    #     clip_news_version_2 = fitz.Rect(70, 130, 265, 160)
    ...
    #     clip_report_version_2 = fitz.Rect(495, 141, 550, 175)
    #     try:
    #         text_version_2 = page.get_text(clip=clip_report_version_2).strip()
    ...
    def get_author_report_version_1(self, page):
        clip_report_version_1 = fitz.Rect(0, 0, page.rect.width, 230)
        text_report_version_1 = page.get_text(clip=clip_report_version_1, sort=True).strip()
        try:
            text_report_version_1 = text_report_version_1.split('：')[1].strip()
            return text_report_version_1.split('\n')[0].strip()
    ...
    def get_summary_report_version_1(self, page):
        clip_report_version_1 = fitz.Rect(0, 100, 400, 200)
        text_report_version_1 = page.get_text(clip=clip_report_version_1, sort=True).strip()
        try:
            if '）' in text_report_version_1:

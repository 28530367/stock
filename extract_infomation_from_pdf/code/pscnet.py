...
class Pscnet(ExtractPdf):
    ...
    def __init__(self, file_path):
        ...
        self.possible_rating = ['中立(維持評等)', '買進(調升評等)', '買進(維持評等)', '降低持股(調降評等)', '中立(調降評等)', '強力買進(調升評等)',
    ...
    def check_version(self, text_check_version):
        if '訪談報告' in text_check_version :
            return 'report' 
        elif '投資速報' in text_check_version :
            return 'news' 
        else: 
            return 'NULL'
    ...
    def get_rating_report_version_1(self, page):
        clip_report_version_1= fitz.Rect(375, 120, 565, 190)
        text_report_version_1 = page.get_text(clip=clip_report_version_1, sort=True).strip()
        try:
            return text_report_version_1.split('\n')[0].strip()
        except:
    ...
    def get_rating_report_version_2(self, page):
        clip_report_version_2 = fitz.Rect(375, 120, 565, 160)
    ...
    def get_tp_report_version_1(self, page):
        clip_report_version_1 = fitz.Rect(380, 210, 560, 320)
        text_report_version_1 = page.get_text(clip=clip_report_version_1, sort=True).strip()
        try:
